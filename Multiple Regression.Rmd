...
This tutorial focused on multiple regression. A procedure where we extend concepts of correlation and simple linear regression to models with multiple predictor variables (but still only one outcome variable).
...
### Small dataset examples (Correlation)

Multiple regression, like simple regression, is very closely related to correlation. As we'll see, many of the approaches we learned for correlation are the basis for multiple regression approaches. 
...
## R Code Correlation and Covariance

I placed the data we just calculated in a file called *xy* with variables called **x** and **y**. The code below runs each analysis. The commands cov and cor correspond to the covariance and correlation. 

```{r echo=T, echo=T}
cov(xy$x,xy$y)
cor(xy$x,xy$y)
```
## Review of Simple Linear Regression Calculations
...
$\large y\prime=2.0+0.70(1)=2.70$

The table below provides $y\prime$ for all the scores. 

```{r echo=F}
all<-cbind(x,y,devx,devy,devxy)
y_prime<-c(2.7,4.1,5.5,6.9)
...
For multiple regression we have multiple predictor variables (I use x values to represent these). The example that follows demonstrates calculations for two variables. In general, it is not practical to do calculations for more than two variables by hand but it is useful to understand where many of the values come from. The notation here is a bit complex. $b_{y1.2}$ refers to the slope for the first predictor (the .2 can be interpreted as "in the presence of the 2nd predictor.)
...
We won't do an exercise with multiple regression calculations as you likely will never encounter a situations where you perform the calculations. It is, however, useful to return to these formulae 

## Introduction to Multiple Regression with R
...
*denial* is the datafile that includes variables that examine relationships between climate denial and a variety of other beliefs. 
...
In the sections that follow we will run the analysis and examine (and in some cases, introduce) several statistics. 
...
## Interpreting the relationships (Slope)

Below, I have run a multiple regression predicting climate change denial from anti-establishment beliefs, traditional values, openness, and agreeableness. Note that to add additional predictors, we simply need to add them to a the string following the ~ symbol. 
...
Calculating the $R^2$ model statistic requires examining a few additional statistics called Sums of Squares values. The next section addresses these calculations.
...
Below is the example from earlier (with just one predictor to keep it simple), now with the $y-\bar{y}^2$ values added in. Recall that we already calculated $y-\bar{y}^2$ earlier as part of the covariance (noted as devx). As a reminder, $\bar{y}$=5.
...
## Effect Size: Squared Semi Partial Correlation
